...
export function AboutClient({ about, skills, profile, socialLinks }: AboutClientProps) {
    // Parsing skills from database
    const hardSkills = skills.filter(s => s.type === 'hard').map(s => s.name)
    const softSkills = skills.filter(s => s.type === 'soft').map(s => s.name)

    // Use DB data or callbak
    const stats = [
        { icon: Code2, value: about?.yearsExp || "3+", label: "Years Experience" },
        { icon: Database, value: about?.projects || "10+", label: "Projects Completed" },
        { icon: Lightbulb, value: about?.technologies || "15+", label: "Technologies" },
        { icon: Users, value: about?.collaborations || "5+", label: "Team Collaborations" },
    ]

    const paragraph1 = about?.paragraph1 || "I am a final-year Software Engineering Technology student at IPB University with a strong focus on backend development. Experienced in designing efficient database schemas, building secure APIs, and implementing complex business logic with clean architecture principles."
    const paragraph2 = about?.paragraph2 || "Currently working as a Software Engineer Intern at PT Kuantum Solusi Teknologi, developing large-scale budgeting systems. Passionate about creating scalable, high-performance solutions that solve real-world problems."
                        ...
                        >
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {hardSkills.length > 0 ? hardSkills.map((skill, index) => (
                                        <motion.div
                                            key={skill}
                                        ...
                                        <p className="text-muted-foreground text-sm italic">No hard skills added yet.</p>
                                    )}
                                </div>
                            </div>
